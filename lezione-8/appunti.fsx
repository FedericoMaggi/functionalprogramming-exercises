...
// La label ` |@ "whatever" ` -> serve nel test per specificare quale fallisca
...
let prop_listToGlist (xs : char list) =
  xs = (List2GList xs |> GListToList)

do Check.Quick prop_listToGlist

printfn "AST"
// AST - Abstract Syntax Tree (Alberi Sintattici)
// Descrivono la sintassi di un certo linguaggio
// Prendono una grammatica e la implementano


// Creiamo un albero sintattico per un'espressione
// aritmetica che lavora su espressioni aritmentiche 
// di questo tipo:
//
// e ::= n | e1 + e2 | e1 * e2 | e mod 2 
// make sure you understand BNF
// Così definiamo il tipo di espressione aritmetica su cui lavoreremo
type aexp =    
  | C of int
  | Sum of aexp * aexp
  | Prod of aexp * aexp
...
// infix notation
// Definiamo degli operatori creando degli operatori infissi
let (++) m n = Sum(m,n);;
let (@@) m n = Prod(m,n);;
...
(* 
  The expression is represented by the following tree: 
...
printfn "((3 * 5) mod 2 ) + 12 = [%A]" a1

// explain natural semantics: e >> v  and trace ((3 * 5) mod 2 ) + 12 

// Eval scorre l'albero sintattico e sostituisce ai costruttori la valutazione opportuna
// eval  : aexp -> int
let rec eval t  =
    match t with
    | C n         -> n
...
let r1 = eval a1;;
printfn "Eval: ((3 * 5) mod 2 ) + 12 = %d" r1

(* 
  Un esempio di compilazione su AST: semplificazione con 0, 
  basato su equazioni:
...
// OPT ci permette di snellire l' AST precedentemente generato.
// Un compilatore può utilizzare una cosa del genere per ripulire il codice.
// Ad esempio PROD(n,0) è inutile effettuare il prodotto,
// eliminiamo staticamente questo tipo di espressioni.
//
// val opt : e:aexp -> aexp
let rec opt e =
  match e with
...
printfn "Testing optimisation"
do Check.Quick prop_opt

printfn "> Using variables"
(*
...
type aexpv =
 ...
 (*
  ...
  variable. 
  Similarly to what F# does, we use an environment, 
  i.e. a finite map of variables to values, to record and lookup values of vars
  ...
  MAP: Modulo di Mappe Finite per F#
  Utile per dizionari, grafi, ecc.
  E' implementato con un albero bilanciato.
*)

// Creiamo l'environment del nostro expression evaluator per contenere delle variabili.
// Usiamo delle varibili come stringhe "x", "y", "z" e le recuperiamo dalla mappa.
...
// Evaluation of an expression with an environment
let rec aeval t env =
    match t with
    | C n         -> n
    | V s         -> Map.find s env     // Lookup for variables in map
